import {
  type CommerceSeoHead,
  createCategoryHead,
  createCommerceSeo,
  createProductHead,
  serializeJsonLd,
} from "@commercengine/seo";
 ...
 */
export const seo = createCommerceSeo({ storefront, site, routes });

/**
 * Head data for a product or category, or `null` until it resolves.
 *
 * Building it needs route resolution, which is async, so this cannot be computed inline during
 * render. Passing the result to `<SEO head={...} />` keeps the page component declarative.
 */
/**
...
export function useCommerceSeoHead(
  ...
  useEffect(() => {
    ...
    const build =
      kind === "product"
        ? createProductHead(seo, entity as ProductDetail).then((value) =>
            withBreadcrumb(value, entity as ProductDetail)
          )
        : createCategoryHead(seo, entity as Category);
    void build
      .then((value) => {
