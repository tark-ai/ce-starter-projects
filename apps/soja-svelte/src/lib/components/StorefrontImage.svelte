...
function buildSrcSet(
  ...
  const unique = Array.from(entries.entries()).filter(
    ([density, src], index, all) =>
      all.findIndex(([, entrySrc]) => entrySrc === src) === index || density === "1x"
  );

  if (unique.length <= 1) return undefined;
  return unique.map(([density, src]) => `${src} ${density}`).join(", ");
}
