 ...
 *
 * Vite ranks the mode file ABOVE .env.local. Next ranks it below, so the
 * Next starters deliberately order these differently — it is not an
...
export function requireCredentials(env, mode = "production", tag = "commerce") {
  const missing = ["PUBLIC_STORE_ID", "PUBLIC_API_KEY"].filter((name) => !env[name]);
