...
function publicSdk() {
  return storefront.publicStorefront();
}

// --- List Products (simple listing, e.g. home featured row) ---
...
export function useProducts(options: UseProductsOptions = {}): UseProductsResult {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await publicSdk().catalog.listProducts({ page, limit, category_id });
...
export function useListSkus(options: UseListSkusOptions = {}): UseListSkusResult {
  ...
  const rqQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await publicSdk().catalog.listSkus({ page, limit, category_id });
      if (error) throw new Error(error.message);
      return data;
    },
    enabled,
...
export function useSearchProducts(options: UseSearchProductsOptions = {}): UseSearchProductsResult {
  ...
  const rqQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await publicSdk().catalog.searchProducts(body);
      if (error) throw new Error(error.message);
      return data;
    },
    enabled,
...
export function useSimilarProducts(productId: string): UseSimilarProductsResult {
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await publicSdk().catalog.listSimilarProducts({
...
export function useProductDetail(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await publicSdk().catalog.getProductDetail({ product_id: slug });
      if (error) throw new Error(error.message);
      return data;
    },
...
export function useCategories(options: { enabled?: boolean } = {}): UseCategoriesResult {
  ...
  const query = useQuery({
    queryKey: ["categories"],
    queryFn: async () => {
      const { data, error } = await publicSdk().catalog.listCategories();
      if (error) throw new Error(error.message);
      return data;
    },
    staleTime: 5 * 60 * 1000,
